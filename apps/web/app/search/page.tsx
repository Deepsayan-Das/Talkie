...
import { Search, X, UserPlus, MessageCircle, ArrowLeft, Loader2, Bot } from 'lucide-react'
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            >
                                <div
                                    ...
                                    onClick={() => router.push(`/profile/${TALKIE_BOT_PROFILE.id}`)}
                                >
                                    <img
                                        src={TALKIE_BOT_PROFILE.avatar}
                                        alt={TALKIE_BOT_PROFILE.displayName}
                                        className="w-10 h-10 rounded-full object-cover border border-neutral-700 shrink-0"
                                    />
                                    <div className="flex flex-col text-left truncate">
                                        <div className="flex items-center gap-2">
                                            <span className="font-bold text-sm text-neutral-100 hover:underline">
                                                {TALKIE_BOT_PROFILE.displayName}
                                            </span>
                                            <Badge variant="mono">AI BOT</Badge>
                                            <Badge variant="active" dot>Online</Badge>
                                        </div>
                                        <span className="font-mono text-xs text-neutral-500">@{TALKIE_BOT_PROFILE.username}</span>
                                        <p className="text-xs text-neutral-400 truncate mt-0.5">{TALKIE_BOT_PROFILE.bio}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                                    <Button
                                        variant="secondary"
                                        size="sm"
                                        onClick={() => handleMessage(TALKIE_BOT_PROFILE.id)}
                                        leftIcon={<MessageCircle size={14} />}
                                    >
                                        CHAT
                                    </Button>
                                    {requested.has(TALKIE_BOT_PROFILE.id) ? (
                                        <Badge variant="mono">ADDED</Badge>
                                    ) : (
                                        <Button
                                            variant="primary"
                                            size="sm"
                                            onClick={() => handleAdd(TALKIE_BOT_PROFILE.id)}
